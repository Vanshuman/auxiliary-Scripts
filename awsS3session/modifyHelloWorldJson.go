...
func ModifyTheFiles(txtFile string) error {
	for _, r := range ReadFromTxtFile(txtFile) {
		...
		dest := fmt.Sprintf("/Users/anshuman/GolandProjects/awsS3update/HelloWorld/%s/attributes.json", fileName)
		attr, err := InitAttributes(dest)
		if err != nil {
		...
		ModifyAttributesJson(attr, "VOTERID")
...
func writeFiles(attr Attributes, orderId, name string) error {
	// Copying the data to attributes_old.json
	attrLoc := fmt.Sprintf("/Users/anshuman/GolandProjects/awsS3update/awsS3session/FailedVoterID/AddressWVoterID-NEW/%s/%s.json", orderId, name)
